...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let col1 = UIColor(red: 249 / 255, green: 203 / 255, blue: 34 / 255, alpha: 1)
        let col2 = UIColor(red: 247 / 255, green: 54 / 255, blue: 0 / 255, alpha: 1)
        let gradientImage = UIImage.makeGradient(colors: [col1, col2], locations: [0.19, 1], axis: .horizontal, size: CGSize(width: imagesWidth, height: 1))

        vStack.addArrangedSubview(makeImageView(imageURL: Resources.URLs.sampleImage2))
        vStack.addArrangedSubview(makeGradientImageView(gradientImage: gradientImage))

        let filter = CustomColorMap()
        let inputImage = CIImage(contentsOf: Resources.URLs.sampleImage2)
